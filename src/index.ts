...
const getCommits = async (
  from = "c76319b301d20f2fbbf6585bfe7e9dc466b1f99f",
  to = "HEAD",
  paths = ["--", "../commerce"]
) => {
...
const argv = yargs(process.argv.slice(2))
  ...
  .usage("$0 <cmd> [args]")
  .command("detect", "Detect changes", (yargs) => {
    yargs
      .option("from", {
  ...
  async (argv) => {
    // @ts-ignore
    const paths = argv.path ? ["--", argv.path] : [];
    const commits = await getCommits(argv.from, argv.to, paths);
